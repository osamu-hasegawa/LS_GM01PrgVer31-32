...
'   update: 32-V3.80506B  2013.10.07 s.f  ì˜å˙ï‚ê≥Å@Å}0.05Å®+0.05/-0.5Ç÷Å@ïœçX
'
Option Explicit
Dim lK1%, lK2%, lK3%, lK4%, lK5%, lK6%, lK7%, lK8%
Dim lK9%, lK10%, lK11%, lK12%, lK13%, lK14%
...
' ---------------------- êßå‰ÉRÉ}ÉìÉh
  For i = 0 To 200
    ...
    Case "S"
      ...
      itp = itp + 1
      If itp < 7 Then
        dt = sisub(i): DispText4 itp, dt, True     'â∑ìxÇP
        dt = sjsub(i): DispText5 itp, dt, True     'â∑ìxÇQ
        dt = sksub(i): DispText6 itp, dt, True     'è∏â∑éûä‘
      End If
    Case "L"
      ilp = 7
...
' ---- kata Shotêî ï\é¶
    For i = 0 To 7                  ' Text17 ÇÕÅA0Å`7Å@Ç‹Ç≈
...
' ---------------------- êßå‰ÉRÉ}ÉìÉh
  For i = 0 To 200
    ...
    Case "S"
      ...
      itp = itp + 1
      If itp < 7 Then
        sisub(i) = Val(Text4(itp - 1).Text)    'â∑ìxÇP
        sjsub(i) = Val(Text5(itp - 1).Text)    'â∑ìxÇQ
        sksub(i) = Val(Text6(itp - 1).Text)    'è∏â∑éûä‘
      End If
    Case "L"
      ilp = 7
...
'  --- kataNo. ---
    ...
    Next i
'  --- kata Shotêî ---
    For i = 0 To 7                  'Å@Text17ÇÕ0Å`7Ç‹Ç≈
...
Private Sub Command3_Click(Index As Integer)
       If (iflgKataTorF(Index) = True) Then
          iflgKataTorF(Index) = False              'Å@¿ﬁ–∞ÇÕîñâ©óŒÅ@false
          Command3(Index).BackColor = CmndColon(3)  'on 3=Å@îñóŒ
         Else
          iflgKataTorF(Index) = True
          Command3(Index).BackColor = CmndColoff(1)  '  off 1=äDêF
       End If
End Sub
...
'  Henkou_No = 0    'ïœçXÇÕÅALS21_SCë§Ç≈
  coxDtSet
  SetData
  lFlgDisp = True
  For i = 0 To 9
...
Private Sub DispText5(i%, dt!, flg%)    'Å@Å@â∑ìxÇQ
  ...
  End If
End Sub
Private Sub DispText6(i%, dt!, flg%)    'Å@Å@è∏â∑éûä‘
  If flg = False Then
...
'    VScroll12(j).max = -0.05 * lK12
    VScroll12(j).max = -0.5 * lK12           ' -0.05 Å® -0.5 2013.10.07ïœçX
